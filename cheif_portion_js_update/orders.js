...
// Finds an order by its id number.
...
// Moves an order from "queued" to "prep" (sent to the kitchen).
...
// Shows a simple popup with the order's items, standing in for a
// full recipe page (that will link to the real Recipes page later).
...
// ----------------------------------------------------------------------
// PART 9: FILTER PILLS (All / Locked / Unlocked) AND SORT BUTTON
// ----------------------------------------------------------------------
...
for (var f = 0; f < filterButtons.length; f++) {
  filterButtons[f].addEventListener("click", function (event) {
    // Take the "active" look off every filter button...
    for (var g = 0; g < filterButtons.length; g++) {
      filterButtons[g].classList.remove("active");
    }
    // ...then put it back on just the one that was clicked.
...
// The "Release Time ⌄" button flips between soonest-first and
// latest-first sorting, and flips its little arrow to match.
...
// The "How gating works" pill opens a popup that explains, in plain
// words, what LOCKED / UPCOMING / READY TO START mean.
...
// ----------------------------------------------------------------------
// PART 10: THE POPUP WINDOW (MODAL) - CLOSING IT
// ----------------------------------------------------------------------

// Close the modal if the chef clicks the × button inside it (using
// one click listener on the whole page, since the × button is
// re-created every time we open a new popup).
...
// Also close the modal if the chef clicks the dark background
// outside the white popup box.
...
// ----------------------------------------------------------------------
// PART 11: THE HEADER (profile picture menu + shift button)
// Every page has this same little header, so this same block of
// code will be repeated (kept identical) in each page's JS file.
// ----------------------------------------------------------------------

// Clicking the round avatar circle opens/closes the account menu.
...
// "Account details" and "Switch chef" are not hooked up to a real
// account system yet, so we just show a friendly toast message.
...
// The green/grey "Off Shift" pill button at the top. Clicking it
// turns the shift on or off, and this is remembered so every page
// shows the same shift status.
...
if (shiftChipButton) {
  ...
  // Read the saved shift status when the page first opens.
...
// Redraw every 30 seconds so the countdowns (like "Unlocks in 12m")
// keep counting down even if the chef leaves the page open.
