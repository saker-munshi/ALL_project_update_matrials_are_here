// ======================================================================
// RUSTIC FORK - RECIPES PAGE SCRIPT
// File: recipes.js
//
// IMPORTANT: This file does NOT change any HTML or CSS.
// It fills in the empty boxes already in recipes.html (like
// <div id="recipeGrid"></div> and <div id="categories"></div>) and
// makes the buttons that are already on the page actually work.
//
// What this file does, in plain words:
//   1. Makes a small list of pretend recipes and remembers them in
//      the browser (localStorage), so they survive a page refresh.
//   2. Draws the category pills at the top ("All 15", "Pizza 2"...)
//      by counting the recipes itself - the numbers are never typed
//      in by hand.
//   3. Draws every recipe card in the grid.
//   4. Makes the search box and category pills filter the grid.
//   5. Makes "+ Add Recipe" open a blank form in the popup window.
//   6. Makes "Edit" open the same form, already filled in.
//   7. Makes "Delete" remove a recipe (after asking "are you sure").
//   8. Makes the shared header (profile menu + shift button) work.
// ======================================================================
...
// The notebook page name we use to save our recipes in the browser.
var RECIPES_STORAGE_KEY = "rf_recipes";

// Same shared shift key used on every page.
var SHIFT_STORAGE_KEY = "rf_on_shift";

// The order we want the category pills to appear in.
var CATEGORY_ORDER = ["Appetizers", "Pizza", "Drinks", "Desserts", "Burgers", "Mains"];

// Real food photographs for the recipe cards.
// Only the image source is changed; the rest of the recipe data and
// page behavior stays exactly the same.
var RECIPE_IMAGES = {
  ...
  "Crème Brûlée": "https://images.unsplash.com/photo-1551024506-0bccd828d307?auto=format&fit=crop&w=900&q=85",
...
// Fallback real photos for newly added recipes. Existing recipes above
// keep their own matching photograph.
var CATEGORY_IMAGES = {
...
// What the chef has typed in the search box, and which category
// pill is currently selected ("all" means show every category).
var recipeSearchText = "";
var selectedCategory = "all";

// While the Add/Edit popup is open, this remembers which recipe we
// are editing. It is null when we are adding a brand new recipe.
...
// ----------------------------------------------------------------------
// PART 3: LOADING AND SAVING RECIPES
// ----------------------------------------------------------------------
...
// The very first set of recipes, used only the first time this page
// is ever opened on this computer. NOTE: these names and times are
// simple stand-ins (we do not have the original recipe photos or
// exact numbers), but the categories/difficulty/count add up the
// same way your screenshots showed: 15 recipes total.
...
// ----------------------------------------------------------------------
// PART 4: FILTERING (search box + category pill)
// ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
// PART 5: BUILDING THE CATEGORY PILLS
// ----------------------------------------------------------------------

function renderCategoryPills() {
  // Count how many recipes are in each category, ourselves, instead
  // of writing the numbers in by hand.
  ...
  // One pill per category, only if that category actually has recipes.
...
// ----------------------------------------------------------------------
// PART 6: BUILDING ONE RECIPE CARD
// ----------------------------------------------------------------------

function buildRecipeCardHtml(recipe) {
  var imageUrl = RECIPE_IMAGES[recipe.name] || CATEGORY_IMAGES[recipe.category] || "";

  // The difficulty word gets its own CSS class for color:
  // .easy (green), .medium (yellow/brown) or .hard (red).
...
// ----------------------------------------------------------------------
// PART 7: THE MAIN RENDER FUNCTION
// ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
// PART 8: SEARCH BOX + CATEGORY PILL CLICKS
// ----------------------------------------------------------------------
...
// The category pills get rebuilt every render, so we listen on their
// parent box instead of on each pill directly.
...
// ----------------------------------------------------------------------
// PART 9: EDIT / DELETE CLICKS ON A RECIPE CARD
// ----------------------------------------------------------------------
...
function deleteRecipe(recipeId) {
  ...
  // A plain "are you sure?" box - simple and built into every
  // browser, so we do not need to build our own.
...
// ----------------------------------------------------------------------
// PART 10: THE ADD / EDIT RECIPE FORM (shown inside the popup window)
// ----------------------------------------------------------------------

// Builds the <option> list for a <select>, marking the current value
// as selected.
...
// Opens the popup with a blank form (Add) or a filled-in form (Edit),
// depending on whether recipeId is given.
function openRecipeForm(recipeId) {
  ...
  // Hook up the Save button every time we open the form, since the
  // form's HTML (and this button) is brand new each time.
  document.getElementById("rfSaveBtn").addEventListener("click", saveRecipeForm);
}

// Reads whatever the chef typed into the form and saves it.
function saveRecipeForm() {
  ...
  if (!minutes || minutes < 1) {
    minutes = 5; // a safe default so the card always has a real number
  }

  if (recipeBeingEditedId) {
    // We are editing an existing recipe - find it and update it.
    ...
    // We are adding a brand new recipe. Give it an id number that
...
// The "+ Add Recipe" button at the top of the page always opens a
// blank form.
...
// ----------------------------------------------------------------------
// PART 12: THE HEADER (profile picture menu + shift button)
// Same block used on every page, kept identical.
// ----------------------------------------------------------------------
